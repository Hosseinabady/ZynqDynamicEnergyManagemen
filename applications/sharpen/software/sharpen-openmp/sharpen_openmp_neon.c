...
void kernel_sw_openmp(float* inputImage, float* restrict  outImage) {
...
int main(int argc, char** argv)
{
	...
	for (int i = 0; i < SOFTWARE_ITERATION; i++) {
		kernel_sw_openmp(image_sw, outImage_sw);
	}
...
//	POWER_MONITORING_EPILOGUE();
	...
	printf("OpenMP+NEON software execution time  %.6lf ms elapsed\n", software_execution_time);

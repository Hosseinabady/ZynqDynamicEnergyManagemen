...
void init_array(float* a, float* b)
{
	...
	for (i = 0; i < DATA_LENGTH; i++) {
		a[i] = rand()%200+rand()/(float)RAND_MAX;//row
		b[i] = rand()%200+rand()/(float)RAND_MAX;//row
	}
...
//	for (int i = 0; i < SOFTWARE_ITERATION; i++) {
	...
	VOLTAGE_SCALING_PROLOGUE(0.65, 0.69, 0.01);
...
void refresh_hardware(int *accel_status) {
	...
	if (*accel_status == 0) {
		...
		//sprintf(cmd, "cat vector_add_stream_2X.bin > /dev/xdevcfg");
		sprintf(cmd, "cat vector_add_stream_2X.bin > /dev/xdevcfg");
		run_command(cmd);

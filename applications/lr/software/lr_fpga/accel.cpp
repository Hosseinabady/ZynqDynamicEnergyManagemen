...
//	printf("accel_write: ceck point 1\n");
	...
	rd_command->register_physical_offset_address = XLR_CONTROL_BUS_ADDR_IN_X_DATA;
	rd_command->value =  (u32)X/4;

	ioctl ( file_desc, FPGACL_DEVICE_IOWRITE, rd_command);
  	//---------------------------------------
	rd_command->device_physical_base_address = ACCELERATOR;
	rd_command->register_physical_offset_address = XLR_CONTROL_BUS_ADDR_IN_Y_DATA;
	rd_command->value = (u32)Y/4;

	ioctl ( file_desc, FPGACL_DEVICE_IOWRITE, rd_command);
  	//---------------------------------------
	rd_command->device_physical_base_address = ACCELERATOR;
	rd_command->register_physical_offset_address = XLR_CONTROL_BUS_ADDR_N_DATA;
	rd_command->value = n;
...
//	printf("accel_write: ceck point 1\n");
  	...
  	//=======================HP0-0 write command========================================================
	rd_command->device_kernel_base_address     = (void *)device_kernel_address;
	rd_command->register_physical_offset_address = XLR_CONTROL_BUS_ADDR_A0_DATA;

	value.u = ioctl ( file_desc, FPGACL_DEVICE_IOREAD, rd_command);
	*A0 = value.f;
  	//---------------------------------------
	rd_command->device_physical_base_address = ACCELERATOR;
	rd_command->register_physical_offset_address = XLR_CONTROL_BUS_ADDR_A1_DATA;
...
//	printf("accel_start: ceck point 1\n");
	rd_command->device_kernel_base_address     = (void *)device_kernel_address;
	rd_command->register_physical_offset_address = XLR_CONTROL_BUS_ADDR_GIE;
	rd_command->value = 0;
  	ioctl ( file_desc, FPGACL_INTERRUPT_ENABLE, rd_command);

  	rd_command->device_kernel_base_address     = (void *)device_kernel_address;
	rd_command->register_physical_offset_address = XLR_CONTROL_BUS_ADDR_AP_CTRL;
	rd_command->value = 0;

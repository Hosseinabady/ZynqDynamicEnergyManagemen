...
void init_array(DATA_TYPE* a, DATA_TYPE* b)
{
	...
	for (i = 0; i < DATA_LENGTH; i++) {
		a[i] = rand()/(DATA_TYPE)RAND_MAX;//row
		b[i] = rand()/(DATA_TYPE)RAND_MAX;//row
	}
...
void kernel_sw(DATA_TYPE* a, DATA_TYPE* b, DATA_TYPE* c) {
...
void kernel_hw(DATA_TYPE* a, DATA_TYPE* b, DATA_TYPE* c) {
...
int validate(DATA_TYPE* c_sw, DATA_TYPE* c_hw ) {
	int i;
...
int main(int argc, char** argv)
{
	...
	DATA_TYPE* a_sw = (DATA_TYPE *)malloc(sizeof(DATA_TYPE)*DATA_LENGTH);
	DATA_TYPE* b_sw = (DATA_TYPE *)malloc(sizeof(DATA_TYPE)*DATA_LENGTH);
	DATA_TYPE* c_sw = (DATA_TYPE *)malloc(sizeof(DATA_TYPE)*DATA_LENGTH);
	DATA_TYPE* c_hw_result = (DATA_TYPE *)malloc(sizeof(DATA_TYPE)*DATA_LENGTH);
...
//	for (int i = 0; i < SOFTWARE_ITERATION; i++) {
	...
	fpgacl_memread((void*)c_hw_result, (u32 *)c_hw, sizeof(DATA_TYPE)*DATA_LENGTH);
...
void refresh_hardware(int *accel_status) {
	...
	if (*accel_status == 0) {
		...
		sprintf(cmd, "cat vector_add_stream_64b.bin > /dev/xdevcfg");
		run_command(cmd);

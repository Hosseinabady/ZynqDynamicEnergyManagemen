...
int main(int argc, char** argv)
{
    ...
    AXI_PIXEL* in_pix_padded  = (AXI_PIXEL *)malloc((MAX_HEIGHT+2)*MAX_WIDTH*sizeof(AXI_PIXEL));
    AXI_PIXEL* out_pix_padded = (AXI_PIXEL *)malloc((MAX_HEIGHT+2)*MAX_WIDTH*sizeof(AXI_PIXEL));

    AXI_PIXEL* in_pix  = in_pix_padded  + MAX_WIDTH;
    AXI_PIXEL* out_pix = out_pix_padded + MAX_WIDTH;
    ...
    // Hardware accelerator works on video pixel streams
     for(x = 0; x < MAX_HEIGHT; x++){
      for(y = 0; y < MAX_WIDTH; y++){
        ...
        in_pix[x*MAX_WIDTH + y] = pixel.R;
        in_pix[x*MAX_WIDTH + y] = in_pix[x*MAX_WIDTH + y] << 8;
        in_pix[x*MAX_WIDTH + y] = in_pix[x*MAX_WIDTH + y] | pixel.G;
        in_pix[x*MAX_WIDTH + y] = in_pix[x*MAX_WIDTH + y] << 8;
...
void refresh_hardware(int *accel_status) {
	...
	if (*accel_status == 0) {
		...
		sprintf(cmd, "cat  sobel_filter_4X.bin > /dev/xdevcfg");
		run_command(cmd);

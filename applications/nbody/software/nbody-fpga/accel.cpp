...
void accel_epilogue() {
	...
	printf("check point accel_epilogue 2\n");
	fpgacl_close();
	printf("check point accel_epilogue 3\n");
	close(file_desc);
	printf("check point accel_epilogue 4\n");
...
//	printf("accel_write: ceck point 1\n");
	...
	rd_command->register_physical_offset_address = XNBODY_CONTROL_BUS_ADDR_POSITIONSIN_OFFSET_DATA;
	rd_command->value = (u32)(positionsIn)/4;

	ioctl ( file_desc, FPGACL_DEVICE_IOWRITE, rd_command);
  	//---------------------------------------

	rd_command->device_kernel_base_address     = (void *)device_kernel_address;
	rd_command->register_physical_offset_address = XNBODY_CONTROL_BUS_ADDR_POSITIONSOUT_OFFSET_DATA;
	rd_command->value = (u32)(positionsOut)/4;

	ioctl ( file_desc, FPGACL_DEVICE_IOWRITE, rd_command);
  	//---------------------------------------
	rd_command->device_kernel_base_address     = (void *)device_kernel_address;
	rd_command->register_physical_offset_address = XNBODY_CONTROL_BUS_ADDR_VELOCITIESIN_OFFSET_DATA;
	rd_command->value = (u32)(velocitiesIn)/4;

	ioctl ( file_desc, FPGACL_DEVICE_IOWRITE, rd_command);
  	//---------------------------------------
	rd_command->device_kernel_base_address     = (void *)device_kernel_address;
	rd_command->register_physical_offset_address = XNBODY_CONTROL_BUS_ADDR_VELOCITIESOUT_OFFSET_DATA;
	rd_command->value = (u32)(velocitiesOut)/4;
...
//	printf("accel_start: ceck point 1\n");
	rd_command->device_kernel_base_address     = (void *)device_kernel_address;
	rd_command->register_physical_offset_address = XNBODY_CONTROL_BUS_ADDR_GIE;
	rd_command->value = 0;
  	ioctl ( file_desc, FPGACL_INTERRUPT_ENABLE, rd_command);

	rd_command->device_physical_base_address = ACCELERATOR;
	rd_command->register_physical_offset_address = XNBODY_CONTROL_BUS_ADDR_AP_CTRL;
	rd_command->value = 0;

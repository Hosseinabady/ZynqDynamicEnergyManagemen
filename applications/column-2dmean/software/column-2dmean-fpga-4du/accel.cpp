...
void  accel_write(	DATA_TYPE *  data,
					DATA_TYPE *  mean,
					int          n
				 )
{
...
//	printf("accel_write: ceck point 1\n");
	...
	rd_command->register_physical_offset_address = XCOLUMN_2DMEAN_CONTROL_BUS_ADDR_DATA_OFFSET_DATA;
	rd_command->value = (u32)data/4;

	ioctl ( file_desc, FPGACL_DEVICE_IOWRITE, rd_command);

  	//---------------------------------------
	rd_command->device_kernel_base_address     = (void *)device_kernel_address;
	rd_command->register_physical_offset_address = XCOLUMN_2DMEAN_CONTROL_BUS_ADDR_MEAN_OFFSET_DATA;
	rd_command->value = (u32)mean/4;
	...
	rd_command->device_kernel_base_address     = (void *)device_kernel_address;
	rd_command->register_physical_offset_address = XCOLUMN_2DMEAN_CONTROL_BUS_ADDR_N_DATA;
...
//	printf("accel_start: ceck point 1\n");
	rd_command->device_kernel_base_address     = (void *)device_kernel_address;
	rd_command->register_physical_offset_address = XCOLUMN_2DMEAN_CONTROL_BUS_ADDR_GIE;
	rd_command->value = 0;
  	ioctl ( file_desc, FPGACL_INTERRUPT_ENABLE, rd_command);

  	rd_command->device_kernel_base_address     = (void *)device_kernel_address;
	rd_command->register_physical_offset_address = XCOLUMN_2DMEAN_CONTROL_BUS_ADDR_AP_CTRL;
	rd_command->value = 0;

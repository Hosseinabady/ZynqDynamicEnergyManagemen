...
void mxv_hw(int n, int m, int p, DATA_TYPE  *a, DATA_TYPE  *b,	DATA_TYPE  *c) {
	...
	matrix_mult(memory, memory, memory, memory, memory, memory, memory, memory, memory, a_offset_address, b_offset_address, c_offset_address);
...
int validate(int n,
	...
	for(i = 0; i < n; i++) {
		for(j = 0; j < p; j++) {
			...
			if(diff != 0){
				printf("Error in out_data at A_hw[%d][%d] = %d != A_sw[%d][%d] = %d\n", i, j, hw, i, j, sw);
				return -1;
			}
		}
	}

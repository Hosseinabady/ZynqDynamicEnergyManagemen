...
//	printf("accel_write: ceck point 1\n");
	...
	rd_command->register_physical_offset_address = XMATRIX_MULT_CONTROL_BUS_ADDR_A_OFFSET_ADDRESS_DATA;
	rd_command->value = (u32)(a)/4;

	ioctl ( file_desc, FPGACL_DEVICE_IOWRITE, rd_command);
  	//---------------------------------------
	rd_command->device_kernel_base_address     = (void *)device_kernel_address;
	rd_command->register_physical_offset_address = XMATRIX_MULT_CONTROL_BUS_ADDR_B_OFFSET_ADDRESS_DATA;
	rd_command->value = (u32)(b)/4;

	ioctl ( file_desc, FPGACL_DEVICE_IOWRITE, rd_command);
  	//---------------------------------------
	rd_command->device_kernel_base_address     = (void *)device_kernel_address;
	rd_command->register_physical_offset_address = XMATRIX_MULT_CONTROL_BUS_ADDR_C_OFFSET_ADDRESS_DATA;
...
//	printf("accel_start: ceck point 1\n");
	rd_command->device_kernel_base_address     = (void *)device_kernel_address;
	rd_command->register_physical_offset_address = XMATRIX_MULT_CONTROL_BUS_ADDR_GIE;
	rd_command->value = 0;
  	ioctl ( file_desc, FPGACL_INTERRUPT_ENABLE, rd_command);

  	rd_command->device_kernel_base_address     = (void *)device_kernel_address;
	rd_command->register_physical_offset_address = XMATRIX_MULT_CONTROL_BUS_ADDR_AP_CTRL;
	rd_command->value = 0;

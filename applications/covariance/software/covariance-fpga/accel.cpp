...
//	printf("accel_write: ceck point 1\n");
	...
	rd_command->register_physical_offset_address = XCOVARIANCE_CONTROL_BUS_ADDR_DATA_OFFSET_DATA;
	rd_command->value = (u32)(data)/sizeof(DATA_TYPE);

	ioctl ( file_desc, FPGACL_DEVICE_IOWRITE, rd_command);
  	//---------------------------------------
	rd_command->device_kernel_base_address     = (void *)device_kernel_address;
	rd_command->register_physical_offset_address = XCOVARIANCE_CONTROL_BUS_ADDR_COV_OFFSET_DATA;
	rd_command->value = (u32)(cov)/sizeof(DATA_TYPE);

	ioctl ( file_desc, FPGACL_DEVICE_IOWRITE, rd_command);
  	//---------------------------------------
	rd_command->device_kernel_base_address     = (void *)device_kernel_address;
	rd_command->register_physical_offset_address = XCOVARIANCE_CONTROL_BUS_ADDR_N_DATA;
	rd_command->value = (u32)(n);

	ioctl ( file_desc, FPGACL_DEVICE_IOWRITE, rd_command);
  	//---------------------------------------
	rd_command->device_kernel_base_address     = (void *)device_kernel_address;
	rd_command->register_physical_offset_address = XCOVARIANCE_CONTROL_BUS_ADDR_M_DATA;
...
//	printf("accel_start: ceck point 1\n");
	rd_command->device_kernel_base_address     = (void *)device_kernel_address;
	rd_command->register_physical_offset_address = XCOVARIANCE_CONTROL_BUS_ADDR_GIE;
	rd_command->value = 0;
  	ioctl ( file_desc, FPGACL_INTERRUPT_ENABLE, rd_command);

  	rd_command->device_kernel_base_address     = (void *)device_kernel_address;
	rd_command->register_physical_offset_address = XCOVARIANCE_CONTROL_BUS_ADDR_AP_CTRL;
	rd_command->value = 0;
